...
export async function POST(req: NextRequest) {
  try {
    const url = process.env.N8N_VOCAB_MARK_UNKNOWN_URL;
    const secret = process.env.N8N_SECRET;
    if (!url || !secret) {
      console.error('[vocab/mark-unknown] Missing env', {
        hasUrl: !!url,
        hasSecret: !!secret,
      });
    ...
    if (!response.ok) {
      ...
      console.error('[vocab/mark-unknown] Upstream error', {
    ...
    console.error('[vocab/mark-unknown] Exception', error);
    return NextResponse.json(
